...
struct GameState {
   ...
   public:
    ...
    uint64_t pieceBit(Piece p) const {
        return pieceBits[p];
    }

    uint64_t pieceAttack(Piece p) const {
        return pieceAttacks[p];
    }

    uint64_t occupancy(int idx) const {
        return occupancies[idx];
    }

    Piece getCapturedPiece(const uint64_t toBit,
                                      const std::vector<Piece> &oppPieces) const;
    Piece pieceAt(sf::Vector2<int> square) const;

    const uint64_t *pieceBitsData() const noexcept {
        return pieceBits;
    }

    const uint64_t *pieceAttacksData() const noexcept {
        return pieceAttacks;
    }

    const uint64_t *occupanciesData() const noexcept {
        return occupancies;
    }
